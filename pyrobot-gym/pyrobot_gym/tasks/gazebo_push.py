...
class LocoBotGazeboPushEnv(locobot_gazebo_env.LocoBotGazeboEnv, utils.EzPickle):
    def __init__(self):
        ...
        LoadYamlFileParamsTest(rospackage_name="pyrobot_rl",
                               ...
                               yaml_file_name="locobot_gazebo_push.yaml")
    ...
    def _get_obs(self):
        ...
        end_effector_velocity = (end_effector_position - self.previous_end_effector_postion)/dt
        # Relative displacement from end-effoctor to desired goal position
        ...
        # Obect Position&Velocity relative to the gripper
        object_rel_pos = object_pos - end_effector_position
        object_rel_velp = object_velp - end_effector_velocity

        # Joint positions and velocities
        joint_positions = self.get_joints_position()
        self.last_joint_positions = joint_positions
        joint_velocities = self.get_joints_velocity()
        ...
        rospy.loginfo('[Get OBS] object_pos = {}'.format(object_pos.ravel()))
    ...
    def _is_done(self, observations):
        ...
        """
        current_obj_pos = observations['observation'][16:19].tolist()

        done = self.check_if_done(
            self.movement_result, self.desired_position, current_obj_pos, self.threshold_error)
        return done
    ...
    def check_if_done(self, movement_result, desired_position, current_obj_pos, threshold_error=1e-02):
        """
        ...
        if movement_result:
            ...
            if distance < abs(threshold_error):
                ...
                rospy.logdebug("Object Still Reaching the desired position")
        else:
    ...
    def calculate_step_reward(self, movement_result, desired_position, current_obj_pos, threshold_error=1e-02):
        """
